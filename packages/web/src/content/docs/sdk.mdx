...
| `session.retry({ path, body })`                            | Retry prompt message                | Reuses an existing latest user message with `body.model`, `body.agent?`, and `body.variant?`; returns <a href={typesUrl}><code>AssistantMessage</code></a> with AI response.                                             |
| `session.retryAsync({ path, body })`                       | Retry prompt message asynchronously | Reuses an existing latest user message with `body.model`, `body.agent?`, and `body.variant?`; returns `204 No Content`.                                                                                                  |
