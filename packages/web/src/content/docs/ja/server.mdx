...
| `POST`   | `/session/:id/message/:messageID/retry`       | 最新の user message を再試行して応答を待ちます      | 本文: `{ model, agent?, variant? }`。既存の prompt parts を再利用し、`{ info: `<a href={typesUrl}><code>Message</code></a>`, parts: `<a href={typesUrl}><code>Part[]</code></a>`}` を返します      |
| `POST`   | `/session/:id/message/:messageID/retry_async` | 最新の user message を非同期に再試行する (待機なし) | 本文: `{ model, agent?, variant? }`。既存の prompt parts を再利用し、`204 No Content` を返します。                                                                                                 |
