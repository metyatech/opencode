...
| `session.retry({ path, body })`                            | プロンプトメッセージを再試行する         | 最新の既存 user message を `body.model`、`body.agent?`、`body.variant?` で再利用し、AI 応答を含む <a href={typesUrl}><code>AssistantMessage</code></a> を返します。                                                                         |
| `session.retryAsync({ path, body })`                       | プロンプトメッセージを非同期に再試行する | 最新の既存 user message を `body.model`、`body.agent?`、`body.variant?` で再利用し、`204 No Content` を返します。                                                                                                                           |
