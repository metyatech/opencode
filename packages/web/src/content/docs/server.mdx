...
| `POST` | `/session/:id/message/:messageID/retry`       | Retry the latest user message and wait for response    | body: `{ model, agent?, variant? }`, reuses existing prompt parts and returns `{ info: `<a href={typesUrl}>Message</a>`, parts: `<a href={typesUrl}>Part[]</a>`}`     |
| `POST` | `/session/:id/message/:messageID/retry_async` | Retry the latest user message asynchronously (no wait) | body: `{ model, agent?, variant? }`, reuses existing prompt parts and returns `204 No Content`                                                                        |
