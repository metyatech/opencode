...
Usage notes:
  ...
  - When a command returns a background process handle, do NOT run the same command again to wait for it. Use the process tool with {"action":"poll","handle":"<handle>","cursor":0,"wait_ms":300000}; then pass the returned next_cursor on subsequent polls. A poll result with wait_status "timeout" and info.state "running" is not an error: the process is still running, so poll again with the returned next_cursor or stop it only if you intend to terminate it.
