...
Usage notes:
  ...
  - When the shell returns a background process handle, use the process tool's \`poll\` action (with \`{"action":"poll","handle":"<handle>","cursor":0,"wait_ms":300000}\` for long polls) to read new output, and pass the returned result's \`next_cursor\` as \`cursor\` on each subsequent poll. Do not re-run the original command just to wait for completion; that starts a second process. If a poll returns wait_status:"timeout" with state running, the command is still running; poll again with the returned next_cursor. Use \`stop\` only if you intend to terminate it.
