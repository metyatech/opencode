...
Usage notes:
  ...
  - When the shell returns a background process handle, use the process tool's \`poll\` action (with \`wait_ms:300000\` for long polls) to read new output, and pass the previous result's \`next_cursor\` as \`cursor\` on each subsequent poll. A \`wait_status\` of \`timeout\` with state \`running\` is not a failure; poll again with the same cursor to keep waiting. Do not re-run the command — the previous run is still alive in the manager. Use \`stop\` only if you intend to terminate it.
