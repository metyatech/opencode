...
To wait for a long-running command to finish, poll with wait_ms:300000. Do not re-run the original shell command just to wait for completion; that starts a second process. A poll with wait_ms waits until new output arrives, the process exits, or the wait window elapses (whichever comes first). An omitted wait_ms (or wait_ms:0) returns immediately. On each subsequent poll, pass the previous result's next_cursor as cursor so you only receive new output. The result includes wait_status: "immediate" (returned at once), "output" (new output), "terminal" (process ended), or "timeout". A "terminal" result only means the process has exited or stopped; if info.outputClosed is false, stdout/stderr may still be draining, so poll again with the returned next_cursor. A "timeout" result with info.state "running" is not a failure: the command is still running and the wait window elapsed, so poll again using that result's next_cursor, or stop it only if you intend to terminate it.
