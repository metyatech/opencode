...
function fake(
  ...
  return {
    ...
    startToolCall: Effect.fn("TestSessionProcessor.startToolCall")(() => Effect.die("unexpected tool call")),
      ...
      return Effect.gen(function* () {
        ...
        yield* Deferred.await(ready).pipe(
          Effect.timeoutOrElse({
            duration: "5 seconds",
            orElse: () => Effect.fail(new Error("retry status was not observed")),
          }),
        )
        ...
        const exit = yield* Fiber.await(fiber).pipe(
          Effect.timeoutOrElse({
            duration: "1 second",
            orElse: () => Effect.fail(new Error("retry backoff interrupt did not finish")),
          }),
        )
