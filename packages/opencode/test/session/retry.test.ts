...
describe("session.retry.retryable", () => {
  ...
  test("does not retry Claude Max subscription issue even when proxy returns 500", () => {
    const error = Schema.decodeUnknownSync(MessageV2.APIError.Schema)(
      new MessageV2.APIError({
        message: "Claude Max subscription issue. Check your subscription status at https://claude.ai/settings/billing.",
        isRetryable: true,
        statusCode: 500,
      }).toObject(),
    )
